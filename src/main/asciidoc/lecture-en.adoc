...
* However, quite often I argue with engineers who consider TestContainers to be an ultimate solution for every problem
* I would like to share some insights based on my personal experience in integration testing in Java
* The takeaways must be useful not just for Java/Kotlin developers
...
* -- And what about external services?.. you can't run them in test containers...
...
^.^|
*Real Systems*
| image:thumbs-up.png[] We have control over the mock and can easily simulate all corner cases.
|
...
image:thumbs-down.png[] "I have Mac M1 and the container is not compatible!"
...
* Paid service
* Requires good Internet connection (won't work on train)
...
|
[none]
* image:thumbs-up.png[] Provide white-box testing (enables verification of calls from the system under test).
* image:thumbs-up.png[] Allow simulation of any state, including failures, facilitating testing of corner cases.
...
Bugs (behavior differing from the real Redis) are constantly being fixed
...
try {
  Awaitility.await().atMost(5, SECONDS)
    ...
    //We should not forget to finalize the execution even in case of errors!
